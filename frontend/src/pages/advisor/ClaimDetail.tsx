  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div>
        <Link
          to="/advisor/claims"
          className="text-sm text-slate-500 hover:text-slate-800 mb-2 inline-block"
        >
          ← Back to Pipeline
        </Link>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-600 mt-1">
              Client:{" "}
              <Link
                to={`/advisor/clients/${claim.client.id}`}
      ...
      {claim.allowed_transitions?.length > 0 && (
        <div className="bg-slate-50 p-4 border border-slate-200 rounded-lg flex flex-wrap gap-2 items-center">
          ...
          {claim.allowed_transitions.map((t: any) => {
            ...
            return (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium rounded ${needsClaimNum ? "bg-slate-200 text-slate-400 cursor-not-allowed" : "bg-yellow-500 text-white hover:bg-yellow-600"}`}
              >
                {t.label}
              </button>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          {/* Incident Details */}
          <section className="bg-white p-5 shadow rounded-lg border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              <div className="md:col-span-2">
                ...
                <p className="font-medium text-slate-900 whitespace-pre-wrap bg-slate-50 p-3 rounded">
          ...
          <section className="bg-white p-5 shadow rounded-lg border border-slate-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6 text-sm">
              <div>
                <p className="text-slate-500 mb-1">Claim Number</p>
                <p className="font-medium text-slate-900">
                  {claim.claim_number || "Pending"}
                </p>
              </div>
              <div>
                <p className="text-slate-500 mb-1">Insurer</p>
                <p className="font-medium text-slate-900">
                  {claim.insurer?.name}
                </p>
              </div>
              <div>
                <p className="text-slate-500 mb-1">Handler Name</p>
                <p className="font-medium text-slate-900">
                  {claim.insurer_details.handler_name || "Unassigned"}
                </p>
              </div>
              <div>
                <p className="text-slate-500 mb-1">Repairer</p>
                <p className="font-medium text-slate-900">
                  {claim.repair.repairer_name || "Not selected"}
                </p>
              </div>
              <div>
                <p className="text-slate-500 mb-1">Hire Car Status</p>
            ...
            <div className="mt-4 flex justify-end">
              <button className="text-xs font-medium text-yellow-600 hover:text-yellow-700">
                Edit Details
              </button>
            </div>
          </section>
            ...
            {!threads || threads.items.length === 0 ? (
              ...
              <div className="space-y-2">
                ...
                {threads.items.length > 3 && (
                  <Link
                    to="/advisor/email"
                    className="text-xs font-medium text-yellow-600 hover:underline block text-center mt-2"
                  >
        ...
        <div className="space-y-6">
          {/* Timeline & Updates */}
          <section className="bg-white p-5 shadow rounded-lg border border-slate-100 flex flex-col h-[600px]">
            <div className="flex justify-between items-center border-b border-slate-100 pb-2 mb-4">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 pr-2">
              {claim.timeline
                ...
                .map((u: any, idx: number) => (
                  ...
                  >
                    <div
                      className={`absolute -left-[5px] top-1 w-2 h-2 rounded-full ${u.type === "status_changed" ? "bg-yellow-500" : "bg-slate-400"}`}
                    ></div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100">
              <div className="space-y-2">
                <textarea
                  ...
                  className="w-full text-sm rounded-md border-slate-300 shadow-sm focus:border-yellow-500 p-2"
                  rows={2}
                />
                <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-yellow-500 text-white text-xs font-medium rounded hover:bg-yellow-600 disabled:opacity-50"
                  >
                    Post
                  </button>
                </div>
