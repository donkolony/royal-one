...
interface ProfileData {
  ...
  adviser: { full_name: string; email: string; phone: string | null } | null;
}
...
function Profile() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setLicenceExpiry(profile.drivers_licence_expiry || '');
    }
  }, [profile]);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="bg-white p-6 rounded-lg shadow border border-gray-100 space-y-6">
        ...
        <div>
          <label className="block text-sm font-medium text-charcoal mb-1">Drivers Licence Expiry</label>
          <div className="flex gap-4">
            <input 
              type="date" 
              value={licenceExpiry}
            ...
            <button 
              ...
              disabled={licenceExpiry === profile.drivers_licence_expiry || updateProfile.isPending}
              className="px-4 py-2 bg-brand-500 text-white rounded font-medium disabled:opacity-50"
            >
              Save
            </button>
      ...
      {profile.adviser && (
        <div className="bg-charcoal text-white p-6 rounded-lg shadow">
          ...
          <p className="font-bold text-lg mb-2">{profile.adviser.full_name}</p>
          <div className="space-y-2 text-sm">
            <p>Email: <a href={`mailto:${profile.adviser.email}`} className="text-brand-500 hover:underline">{profile.adviser.email}</a></p>
            {profile.adviser.phone && <p>Phone: <a href={`tel:${profile.adviser.phone}`} className="text-brand-500 hover:underline">{profile.adviser.phone}</a></p>}
          </div>
        </div>
      )}
    </div>
