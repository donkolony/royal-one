  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-slate-200 pb-2">
        <button onClick={() => setFilter('pending')} className={`px-3 py-1 rounded-full text-sm font-medium ${filter === 'pending' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700'}`}>Pending</button>
        <button onClick={() => setFilter('all')} className={`px-3 py-1 rounded-full text-sm font-medium ${filter === 'all' ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700'}`}>All</button>
      </div>
          ...
          {Object.entries(grouped).map(([urgency, items]) => {
            ...
            return (
              <div key={urgency} className="space-y-4">
                <h3 className="text-lg font-semibold text-slate-800 capitalize border-b border-slate-200 pb-2">
                  {urgency.replace('_', ' ')}
                </h3>
                  ...
                  {items.map((r: any) => (
                    <div key={r.id} className="p-4 flex items-center justify-between hover:bg-slate-50">
                      <div className="flex items-center gap-4">
                        <span className="bg-slate-100 text-slate-700 px-2.5 py-0.5 rounded-full text-xs font-medium truncate max-w-[150px]">
                          {r.client_name}
                        </span>
                        <div>
                          <p className="text-sm font-medium text-slate-900">{r.title}</p>
                          <p className="text-xs text-slate-500">Due: {new Date(r.due_date).toLocaleDateString()} | Type: {r.type.replace('_', ' ')}</p>
                        </div>
                      </div>
                      {r.status === 'pending' && (
                        <button 
