  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/advisor/clients')} className="text-slate-500 hover:text-slate-700">
          ← Back
        </button>
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-6 shadow rounded-lg space-y-4">
            ...
            <h3 className="text-md font-semibold text-slate-800 pt-4 border-t border-slate-100 mt-4">Important Dates</h3>
            <div className="space-y-2 text-sm text-slate-700">
          ...
          <div className="bg-white p-6 shadow rounded-lg space-y-4">
            <h2 className="text-lg font-bold text-slate-900">Overview Dashboard</h2>
            {!dashboard ? <Skeleton className="h-32" /> : (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-3 rounded">
                    <p className="text-xs text-slate-500 font-medium">Net Worth</p>
                    <p className="text-xl font-bold text-slate-900">{formatZAR(dashboard.net_worth.net_worth_cents)}</p>
                  </div>
                  <div className="bg-slate-50 p-3 rounded">
                    <p className="text-xs text-slate-500 font-medium">Monthly Premium</p>
                    <p className="text-xl font-bold text-slate-900">{formatZAR(dashboard.policies.items.reduce((sum, p) => sum + (p.premium_cents || 0), 0))}</p>
                  </div>
                </div>
                
                {dashboard.open_claims.items?.length > 0 && (
                  <div>
                    <h3 className="text-sm font-semibold text-slate-800 mb-2">Recent Claims</h3>
                    <div className="space-y-2">
                      {dashboard.open_claims.items.map((c: any) => (
                        <div key={c.id} className="text-sm flex justify-between p-2 bg-slate-50 rounded">
      ...
      {activeTab === 'goals' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-bold text-slate-900">Financial Goals</h2>
            <button className="px-4 py-2 bg-yellow-500 text-white text-sm font-medium rounded hover:bg-yellow-600">
              Add Goal
            </button>
          </div>
                ...
                return (
                  <div key={g.id} className="bg-white p-4 shadow rounded-lg">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="text-xs bg-slate-100 px-2 py-1 rounded text-slate-600 capitalize">{g.category.replace('_', ' ')}</span>
                    </div>
                    <p className="text-xs text-slate-500 mb-4">{g.description}</p>
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm">
                        <span className="font-medium">{formatZAR(g.current_amount_cents)}</span>
                        <span className="text-slate-500">Target: {formatZAR(g.target_amount_cents)}</span>
                      </div>
      ...
      {activeTab === 'financial' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-bold text-slate-900">Financial Items</h2>
            <button className="px-4 py-2 bg-slate-800 text-white text-sm font-medium rounded hover:bg-slate-700">
              Add Item
            </button>
          </div>
                ...
                <tbody className="bg-white divide-y divide-slate-200">
                  {finItems.map((item: any) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 capitalize">{item.category.replace('_', ' ')}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-right font-medium text-slate-900">{formatZAR(item.amount_cents)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {activeTab === 'reminders' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-lg font-bold text-slate-900">Client Reminders</h2>
            <button className="px-4 py-2 bg-yellow-500 text-white text-sm font-medium rounded hover:bg-yellow-600">
              Add Reminder
            </button>
          </div>
                ...
                <div key={r.id} className="p-4 flex items-center justify-between hover:bg-slate-50">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded capitalize">{r.type.replace('_', ' ')}</span>
                      {r.audience !== 'advisor_only' && <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded">Client Visible</span>}
                      <span className={`text-xs px-2 py-0.5 rounded capitalize ${r.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{r.status}</span>
                    </div>
                    <p className="text-sm font-medium text-slate-900">{r.title}</p>
                    <p className="text-xs text-slate-500">Due: {new Date(r.due_date).toLocaleDateString()}</p>
                  </div>
                  {r.status === 'pending' && (
                    <button className="px-3 py-1.5 text-xs font-medium text-slate-700 border border-slate-300 rounded hover:bg-slate-50">
                      Mark Done
                    </button>
                  )}
                </div>
